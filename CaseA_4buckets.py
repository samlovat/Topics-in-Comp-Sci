...
noUnknown['age'] = pd.cut(noUnknown['age'], bins=bins, labels=ranges, right=True)

# Define the bins and labels (ranges)
bins = [0, 18.5, 25, 30, 35, 40, 50, 65, 80, 100]
ranges = ["0-18.5", "18.5-25", "25-30", "30-35", "35-40", "41-50", "50-65", "65-80", "80-100"]

# Replace values in the "age" column with the specified age ranges
noUnknown['bmi'] = pd.cut(noUnknown['bmi'], bins=bins, labels=ranges, right=True)
...
print('Proportion of strokes in 0 - 25 bin: ', bin25prop, ' || Mean of Bin25: ', bin25mean)
...
ECE = (bin25len/len(predictions))*(abs(bin25prop - bin25mean)) + (bin50len/len(predictions))*abs(bin50prop - bin50mean) + (bin75len/len(predictions))*abs(bin75prop - bin75mean) + (bin100len/len(predictions))*abs(bin100prop - bin100mean)

print('bin25len/len(predictions): ', bin25len/len(predictions), 'bin50len/len(predictions): ', bin50len/len(predictions), 'bin75len/len(predictions): ', bin75len/len(predictions), 'bin100len/len(predictions): ', bin100len/len(predictions))
print('Total coverage: ', bin25len/len(predictions) + bin50len/len(predictions) + bin75len/len(predictions) + bin100len/len(predictions))
